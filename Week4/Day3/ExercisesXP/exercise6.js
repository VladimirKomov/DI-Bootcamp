// Using Javascript, in the <div>, change the value of the id
// attribute from navBar to socialNetworkNavigation, using the setAttribute method.

let element = document.getElementById('navBar');
element.id = 'socialNetworkNavigation';

// We are going to add a new <li> to the <ul>.
//     First, create a new <li> tag (use the createElement method).
//         Create a new text node with “Logout” as its specified text.
//         Append the text node to the newly created list node (<li>).
//             Finally, append this updated list node to the unordered
//             list (<ul>), using the appendChild method.

let newLiElement = document.createElement('li');
let newTextNode = document.createTextNode('Logout');
newLiElement.appendChild(newTextNode);
let listUlElements = document.getElementsByTagName('ul');
let ulElement = listUlElements[listUlElements.length - 1];
ulElement.appendChild(newLiElement);

// Use the firstElementChild and the lastElementChild properties to retrieve the first and last <li>
// elements from their parent element (<ul>). Display the text of each link. (Hint: use the textContent property).

let fistUlElement = document.getElementsByTagName('ul')[0];
let fistListUlElement = fistUlElement.firstElementChild;
console.log(fistListUlElement.textContent);
let lastLiElement = fistUlElement.lastElementChild;
console.log(lastLiElement.textContent);